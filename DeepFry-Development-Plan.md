...
```text
tests/DeepFry.Host.LifecycleTests/Program.cs
```
...
`DeepFry.Protocol` is a shared .NET class library referenced by both
...
- Password data is stored as PBKDF2-SHA256 hash data with a unique random salt,
  ...
  `C:\ProgramData\DeepFry\host-settings.json`.
...
- `scripts\Install-DeepFryClient.ps1` installs the service as Automatic

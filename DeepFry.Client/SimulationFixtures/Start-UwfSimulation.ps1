...
[CmdletBinding()]
param(
    [ValidateSet('FilterOn', 'FilterOff', 'FilterPending')]
    [string]$State = 'FilterOn',
...
if (-not (Test-Path -LiteralPath $ClientExecutablePath)) {
    Write-Host "Executable DeepFry.Client.exe belum ditemukan. Membangun proyek terlebih dahulu..." -ForegroundColor Yellow
    $clientProject = Join-Path $projectRoot 'DeepFry.Client.csproj'
